...
- 266 个基础字段条目（70 个数组类字段 + 196 个标量/mapping 字段），展开后共 290 个字段条目（含 24 个语法变体）。
...
- 多属性场景（`multi-attribute/`）覆盖「数组元素内部多个属性只改其中一个/多个」：9 个字段条目各 4 个场景。
- `ports` 额外包含用户提供的 7 个固定验收场景。
- 当前共 29,676 个 fixture case，其中 1,701 个为上述显式场景（`explicit_cases`），其余为状态矩阵，每个 case 有 5 个 YAML 文件。
...
全量语料从用户意图生成，因此三个实现的残余不匹配就是它们的可观察边界。双行为 intent 引擎的 1,260 个不匹配按根因分为：
...
- `deploy` 的 `generic_resources` 与 `ports` 的 `host_ip`/`protocol`：被修改的值本身是列表 key 的一部分，修改被观察为 add + remove，而不是同一逻辑项的 modify。
- `deploy` devices 的 `capabilities`/`device_ids`：嵌套 set 属性在 device item 内被作为整体值。
