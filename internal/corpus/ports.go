...
type portAcceptanceFixture struct {
	id                        string
	old, user, next, expected []any
}

func portAcceptanceFixtures() []portAcceptanceFixture {
	p := func(published string, target int) any { return fmt.Sprintf("%s:%d", published, target) }
	return []portAcceptanceFixture{
...
func portAcceptanceCases() ([]Case, error) {
	...
	cases := make([]Case, 0, len(portAcceptanceFixtures()))
	for _, fixture := range portAcceptanceFixtures() {
