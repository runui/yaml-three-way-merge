...
//   - deploy reservations generic_resources and ports host_ip/protocol: the
//     changed value is part of the list identity key, so a modification is
//     observed as an add plus a remove rather than a modification of one
//     logical item.
//   - deploy reservations devices: nested set attributes are typed as whole
//     values inside the device item.
//   - service extends: the mapping form is typed as an untyped atomic value.
...
var knownAlgorithmBoundary = map[string]int{
	...
	"services.app.deploy.resources.reservations.devices":           7,
